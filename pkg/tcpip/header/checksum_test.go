...
func TestChecksummableNetworkUpdateAddress(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "UpdateSourceAddressAndChecksum",
			update: header.IPv4.UpdateSourceAddressAndChecksum,
		},
		{
			name:   "UpdateDestinationAddressAndChecksum",
			update: header.IPv4.UpdateDestinationAddressAndChecksum,
		},
	}
			...
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					for _, subTest := range []struct {
						...
						{
							name:   "Update source port with full checksum",
							update: func(h header.ChecksummableTransport) { h.UpdateSourcePortAndMaybeChecksum(newPort, true) },
						},
						{
							name:   "Update destination port with full checksum",
							update: func(h header.ChecksummableTransport) { h.UpdateDestinationPortAndMaybeChecksum(newPort, true) },
						},
					} {
					...
					for _, subTest := range []struct {
						...
						{
							name:   "Update source port without full checksum",
							update: func(h header.ChecksummableTransport) { h.UpdateSourcePortAndMaybeChecksum(newPort, false) },
						},
						{
							name:   "Update destination port without full checksum",
							update: func(h header.ChecksummableTransport) { h.UpdateDestinationPortAndMaybeChecksum(newPort, false) },
						},
					} {
						t.Run(subTest.name, func(t *testing.T) {
							h, _ := test.transportHdr(origSrcPort, origDstPort)
							origXSum := h.Checksum()
							subTest.update(h)
							if got := h.Checksum(); got != origXSum {
								h, _ := test.transportHdr(origSrcPort, origDstPort)
								t.Errorf("got Checksum() = 0x%x, want = 0x%x; h = %#v", got, origXSum, h)
							}
						})
					}
				})
